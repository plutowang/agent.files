...
- **OpenCode Paradigm (Multi-Agent System)**: Host shells here explicitly define JSON-like tool `permission` objects (e.g., `bash: deny`, `edit: allow`). They also contain IDE-specific workarounds, such as isolating the `edit_accuracy/memory.md` fragment strictly to write-enabled agents to prevent execution deadlocks in read-only agents.
