...
3. **Conflict Detection (CRITICAL):** 
   - Does the new prompt contradict an existing universal rule? (e.g., New prompt says "always use `git commit -m`", but our `_core/1_governance/execution_safety.md` says "NEVER run git commit").
