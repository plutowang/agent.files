...
Skip worktree creation entirely if ANY of these hold:

- You are already inside a linked worktree (rung 1).
- You are inside a submodule — treat it as a normal repository (rung 2).
- **You are already on a non-default branch.** If the current branch is anything other than `main`/`master`, build directly on it. Do NOT create a worktree unless the human explicitly asks for one.
- The human declined isolation — work in place.

Only create a worktree when starting from the default branch, and only after asking. Never start implementation on `main`/`master` without explicit human consent.
